...
# Attach a summary op to record accuracy values
tf.summary.scalar('accuracy',accuracy)

# The summary nodes are peripheral to the main computation graph and hence not
# required for computation, these need to be exeuted by explicitly merging the
# nodes and running it.
merged = tf.summary.merge_all()
# FileWriter object to write the summaries to disk
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        # Print training and test set accuracies every 20 steps
        if i % 10 == 0:
            summary, acc = sess.run([merged, accuracy], {x:batch_x, ye:batch_ye})
            # Pass the returned protobuf object to FileWriter to write to disk
    ...
    # all the values. This would be much faster since the dimension isn't so small so as to not make use of the dense matrix
